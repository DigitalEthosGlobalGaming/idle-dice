...
export class PlayerActionButton extends Button {
    ...
    constructor(parent: Panel, action: PlayerAction) {
        super(parent, {
            icon: {
...
export class PlayerActionsUi extends Panel {
    ...
    changeAction(action: PlayerAction) {
        ...
        if (this.currentAction != null) {
            const currentActionButton = this.buttons.find(b => b.action == this.currentAction);
    ...
    onRender(): void {
        ...
        const spacing = 10;
        const totalWidth = this.playerActions.length * (buttonWidth + spacing) - spacing;
        const startX = (width - totalWidth) / 2;

        for (let i = 0; i < this.playerActions.length; i++) {
            const action = this.playerActions[i];
            const button = new PlayerActionButton(this, action);
            button.pos = new ex.Vector(startX + i * (buttonWidth + spacing), height - buttonWidth - 2);
