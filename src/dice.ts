...
const possibleRollAnimations: { [key: string]: {animation:ex.Animation,numbers:number[]}[]}  = {};
...
function getPossibleRollAnimations(faces: number, speed: number) {
    ...
    for (let i = 0; i < faces * rollAnimationsPerFace; i++) {
        ...
        for (const i in diceNumbers) {
            ...
            const duration = ease('diceRollEasing', percentage, diceNumbers.length) * rollDuration;
...
function getRandomRollAnimation(faces: number, speed: number): {
